...
위 목록은 runtime의 canonical import spec 한 곳에서 관리되며, schema allowlist와 runtime package registration은 같은 spec에서 파생된다. 위 7개 stdlib import는 contract가 실제로 import하면 runtime이 Gno stdlib source package와 필요한 내부 dependency를 로드하여 실행한다. 내부 dependency는 runtime 구현 세부이며 contract import allowlist를 넓히지 않는다.

Contract package load/typecheck 단계에서 잘못된 selector, 존재하지 않는 stdlib symbol, 잘못된 호출 형태가 발견되면 `failed to load typed contract package: ...`로 실패한다. 이 메시지는 수정 가능한 핵심 진단 한 줄만 제한된 길이로 남기며, VM stack/dump는 노출하지 않는다. 함수 실행 이후의 runtime panic은 기존 sanitized panic surface를 그대로 사용한다.
